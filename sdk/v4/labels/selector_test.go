...
// Compare to https://github.com/kubernetes/apimachinery/blob/v0.23.5/pkg/labels/selector_test.go
...
func expectMatch(t *testing.T, selector string, ls Set) {
	...
	if m, err := lq.Matches(ls); !m {
		t.Errorf("Wanted %s to match '%s', but it did not: %w\n", selector, ls, err)
	}
}
...
func expectNoMatch(t *testing.T, selector string, ls Set) {
	...
	if m, _ := lq.Matches(ls); m {
		t.Errorf("Wanted '%s' to not match '%s', but it did.", selector, ls)
	}
}

func TestEverything(t *testing.T) {
	if m, err := Everything().Matches(Set{"x": "y"}); !m {
		t.Errorf("Nil selector didn't match: %w", err)
	}
	if !Everything().Empty() {
...
func expectMatchDirect(t *testing.T, selector, ls Set) {
	if m, err := SelectorFromSet(selector).Matches(ls); !m {
		t.Errorf("Wanted %s to match '%s', but it did not: %w.\n", selector, ls, err)
	}
}
...
func expectNoMatchDirect(t *testing.T, selector, ls Set) {
	if m, _ := SelectorFromSet(selector).Matches(ls); m {
		t.Errorf("Wanted '%s' to not match '%s', but it did.", selector, ls)
	}
}
...
func TestRequirementConstructor(t *testing.T) {
	requirementConstructorTests := []struct {
		...
		},
		{
			Key:  "x16",
			Op:   selection.Equals,
			Vals: set.New(strings.Repeat("a", 254)),
			WantErr: &multierror.Error{
				Errors: []error{
					&field.Error{
						Type:     field.ErrorTypeInvalid,
	...
	for _, rc := range requirementConstructorTests {
		...
		if diff := cmp.Diff(rc.WantErr.WrappedErrors(), err, ignoreDetail); diff != "" {
...
func TestToString(t *testing.T) {
	...
	toStringTests := []struct {
		...
		{&internalSelector{
			getRequirement("x", selection.In, set.New("abc", "def"), t),
			getRequirement("y", selection.NotIn, set.New("jkl"), t),
			getRequirement("z", selection.Exists, nil, t)},
			"x in (abc,def),y notin (jkl),z", true},
		{&internalSelector{
			getRequirement("x", selection.NotIn, set.New("abc", "def"), t),
			getRequirement("y", selection.NotEquals, set.New("jkl"), t),
		...
		{&internalSelector{
			...
			"x in (abc,def),", false},
		{&internalSelector{
			getRequirement("x", selection.NotIn, set.New("abc"), t),
			getRequirement("y", selection.In, set.New("jkl", "mno"), t),
			getRequirement("z", selection.NotIn, set.New(""), t)},
			"x notin (abc),y in (jkl,mno),z notin ()", true},
		{&internalSelector{
			getRequirement("x", selection.Equals, set.New("abc"), t),
			getRequirement("y", selection.DoubleEquals, set.New("jkl"), t),
			getRequirement("z", selection.NotEquals, set.New("a"), t),
			getRequirement("z", selection.Exists, nil, t)},
			"x=abc,y==jkl,z!=a,z", true},
		{&internalSelector{
			getRequirement("x", selection.GreaterThan, set.New("2"), t),
			getRequirement("y", selection.LessThan, set.New("8"), t),
			getRequirement("z", selection.Exists, nil, t)},
			"x>2,y<8,z", true},
	}
...
func TestRequirementSelectorMatching(t *testing.T) {
	...
	labelSelectorMatchingTests := []struct {
		...
		{Set{"x": "foo", "y": "baz"}, &internalSelector{
			getRequirement("x", selection.In, set.New("foo"), t),
			getRequirement("y", selection.NotIn, set.New("alpha"), t),
		}, true},
		{Set{"x": "foo", "y": "baz"}, &internalSelector{
			getRequirement("x", selection.In, set.New("foo"), t),
			getRequirement("y", selection.In, set.New("alpha"), t),
		}, false},
		{Set{"y": ""}, &internalSelector{
			getRequirement("x", selection.NotIn, set.New(""), t),
		...
		{Set{"y": "baz"}, &internalSelector{
			getRequirement("x", selection.In, set.New(""), t),
		}, false},
		{Set{"z": "2"}, &internalSelector{
			getRequirement("z", selection.GreaterThan, set.New("1"), t),
		}, true},
	...
	for _, lsm := range labelSelectorMatchingTests {
		if match, err := lsm.Sel.Matches(lsm.Set); match != lsm.Match {
			t.Errorf("%+v.Matches(%#v) => %v, want %v: %w", lsm.Sel, lsm.Set, match, lsm.Match, err)
		}
	}
}
...
func TestSetSelectorParser(t *testing.T) {
	setSelectorParserTests := []struct {
		...
		{"this-is-another-dns.domain.com/key-with-dash in (so,what)", internalSelector{
			getRequirement("this-is-another-dns.domain.com/key-with-dash", selection.In, set.New("so", "what"), t),
		}, true, true},
		{"0.1.2.domain/99 notin (10.10.100.1, tick.tack.clock)", internalSelector{
			getRequirement("0.1.2.domain/99", selection.NotIn, set.New("10.10.100.1", "tick.tack.clock"), t),
		}, true, true},
		{"foo  in	 (abc)", internalSelector{
			getRequirement("foo", selection.In, set.New("abc"), t),
		}, true, true},
		...
		{"x in (abc,)", internalSelector{
			getRequirement("x", selection.In, set.New("abc", ""), t),
		}, true, true},
		...
		{"x notin (abc,,def),bar,z in (),w", internalSelector{
			...
			getRequirement("x", selection.NotIn, set.New("abc", "", "def"), t),
			getRequirement("z", selection.In, set.New(""), t),
		}, true, true},
		{"x,y in (a)", internalSelector{
			getRequirement("y", selection.In, set.New("a"), t),
			getRequirement("x", selection.Exists, nil, t),
		}, false, true},
		{"x=a", internalSelector{
			getRequirement("x", selection.Equals, set.New("a"), t),
		}, true, true},
		{"x>1", internalSelector{
			getRequirement("x", selection.GreaterThan, set.New("1"), t),
		}, true, true},
		{"x<7", internalSelector{
			getRequirement("x", selection.LessThan, set.New("7"), t),
		}, true, true},
		{"x=a,y!=b", internalSelector{
			getRequirement("x", selection.Equals, set.New("a"), t),
			getRequirement("y", selection.NotEquals, set.New("b"), t),
		}, true, true},
		{"x=a,y!=b,z in (h,i,j)", internalSelector{
			getRequirement("x", selection.Equals, set.New("a"), t),
			getRequirement("y", selection.NotEquals, set.New("b"), t),
			getRequirement("z", selection.In, set.New("h", "i", "j"), t),
		}, true, true},
		...
		{"x notin (, a)", internalSelector{
			getRequirement("x", selection.NotIn, set.New("", "a"), t),
		}, true, true},
		...
		{"a in (x,y,notin, z,in)", internalSelector{
			getRequirement("a", selection.In, set.New("in", "notin", "x", "y", "z"), t),
...
func getRequirement(key string, op selection.Operator, vals set.Set[string], t *testing.T) Requirement {
...
func TestValidatedSelectorFromSet(t *testing.T) {
	...
	for _, tc := range tests {
		...
		if diff := cmp.Diff(tc.expectedError.WrappedErrors(), err, ignoreDetail); diff != "" {
