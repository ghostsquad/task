...
// Compare to https://github.com/kubernetes/apimachinery/blob/v0.23.5/pkg/labels/selector.go
...
func (n nothingSelector) Matches(_ Labels) (bool, error)     { return false, nil }
...
func NewRequirement(key string, op selection.Operator, vals []string, opts ...field.PathOption) (*Requirement, error) {
	...
	case selection.In, selection.NotIn:
		if len(vals) == 0 {
			allErrs = multierror.Append(allErrs, field.Invalid(valuePath, vals, "for 'in', 'notin' operators, values set can't be empty"))
		}
	case selection.Equals, selection.DoubleEquals, selection.NotEquals:
		if len(vals) != 1 {
			allErrs = multierror.Append(allErrs, field.Invalid(valuePath, vals, "exact-match compatibility requires one single value"))
		}
	case selection.Exists, selection.DoesNotExist:
		if len(vals) != 0 {
			allErrs = multierror.Append(allErrs, field.Invalid(valuePath, vals, "values set must be empty for exists and does not exist"))
		}
	case selection.GreaterThan, selection.LessThan:
		if len(vals) != 1 {
			allErrs = multierror.Append(allErrs, field.Invalid(valuePath, vals, "for 'Gt', 'Lt' operators, exactly one value is required"))
		}
		for i := range vals {
			if _, err := strconv.ParseInt(vals[i], 10, 64); err != nil {
				allErrs = multierror.Append(allErrs, field.Invalid(valuePath.Index(i), vals[i], "for 'Gt', 'Lt' operators, the value must be an integer"))
			}
		}
	default:
		allErrs = multierror.Append(allErrs, field.NotSupported(path.Child("operator"), op, validRequirementOperators))
	}

	return &Requirement{key: key, operator: op, strValues: vals}, allErrs.ErrorOrNil()
}
...
func (r *Requirement) Matches(ls Labels) (bool, error) {
	...
	case selection.In, selection.Equals, selection.DoubleEquals:
		...
		return r.hasValue(ls.Get(r.key)), nil
	...
	case selection.GreaterThan, selection.LessThan:
		...
		if err != nil {
			return false, fmt.Errorf("ParseInt failed for value %+v in label %+v, %+v", ls.Get(r.key), ls, err)
		}

		// There should be only one strValue in r.strValues, and can be converted to an integer.
		if len(r.strValues) != 1 {
			return false, fmt.Errorf("invalid values count %+v of requirement %#v, for 'Gt', 'Lt' operators, exactly one value is required", len(r.strValues), r)
		}
		...
		for i := range r.strValues {
			...
			if err != nil {
				return false, fmt.Errorf("ParseInt failed for value %+v in requirement %#v, for 'Gt', 'Lt' operators, the value must be an integer", r.strValues[i], r)
			}
		}
		return (r.operator == selection.GreaterThan && lsValue > rValue) || (r.operator == selection.LessThan && lsValue < rValue), nil
	default:
		return false, nil
	}
}
...
func (r *Requirement) Values() set.Set[string] {
	ret := set.New[string]()
	for i := range r.strValues {
...
func (s internalSelector) Matches(l Labels) (bool, error) {
	for ix := range s {
		if matches, err := s[ix].Matches(l); !matches {
			return false, err
		}
	}
	return true, nil
...
func (p *Parser) parseValues() (set.Set[string], error) {
...
func (p *Parser) parseIdentifiersList() (set.Set[string], error) {
	s := set.New[string]()
	for {
		tok, lit := p.consume(Values)
		switch tok {
...
func (p *Parser) parseExactValue() (set.Set[string], error) {
	s := set.New[string]()
